...
function transformationFn(type, check) {
    switch (type) {
        case 'tolower':
            return (data) => data === null || data === void 0 ? void 0 : data.toLowerCase();
        case 'toupper':
            return (data) => data === null || data === void 0 ? void 0 : data.toUpperCase();
        case 'trim':
            return (data) => data === null || data === void 0 ? void 0 : data.trim();
        case 'length':
            return (data) => {
        ...
        case 'startswith':
            return (data) => {
                return data === null || data === void 0 ? void 0 : data.startsWith(prepareLiteral(check, 'Edm.String'));
            };
        case 'endswith':
            return (data) => {
                return data === null || data === void 0 ? void 0 : data.endsWith(prepareLiteral(check, 'Edm.String'));
            };
        case 'substringof':
            return (data) => {
                return (check === null || check === void 0 ? void 0 : check.indexOf(prepareLiteral(data, 'Edm.String'))) !== -1;
            };
        case 'contains':
            return (data) => {
                return (data === null || data === void 0 ? void 0 : data.indexOf(prepareLiteral(check, 'Edm.String'))) !== -1;
            };
        case 'concat':
            return (data) => {
        ...
        case 'indexof':
            return (data) => {
                return data === null || data === void 0 ? void 0 : data.indexOf(prepareLiteral(check, 'Edm.String'));
            };
        case 'substring':
            return (data) => {
                return data === null || data === void 0 ? void 0 : data.substring(check);
            };
        case 'matchesPattern':
 ...
 */
function createTenantDataLoader(mockDataRootFolder, entity, isDraft, dataAccess, fallbackData, generateMockData) {
    return function (contextId) {
        if (dataAccess.fileLoader.syncSupported()) {
            ...
            const targetFile = (0, path_1.join)(mockDataRootFolder, entity + fileNameSuffix) + '.json';
        ...
        if (result.length === 0 && generateMockData) {
            result.__generateMockData = generateMockData;
        }
        return result;
    };
}
...
class MockDataEntitySet {
    static async read(mockDataRootFolder, entity, generateMockData, forceNullableValuesToNull, isDraft, dataAccess) {
        var _a, _b;
        const jsonFilePath = (0, path_1.join)(mockDataRootFolder, entity) + '.json';
        const jsPath = (0, path_1.join)(mockDataRootFolder, entity) + '.js';
        const tsPath = (0, path_1.join)(mockDataRootFolder, entity) + '.ts';
        let outData = [];
        let isInitial = true;
        dataAccess.log.info(`Trying to find ${jsPath} for mockdata`);
        const existJS = await dataAccess.fileLoader.exists(jsPath);
        const existTS = ((_b = (_a = dataAccess.fileLoader).isTypescriptEnabled) === null || _b === void 0 ? void 0 : _b.call(_a)) === true && (await dataAccess.fileLoader.exists(tsPath));
        if (existJS || existTS) {
            try {
        ...
        if ((isInitial || !outData.getInitialDataSet) && (await dataAccess.fileLoader.exists(jsonFilePath))) {
            ...
            try {
                ...
                isInitial = false;
                outData.getInitialDataSet = createTenantDataLoader(mockDataRootFolder, entity, isDraft, dataAccess, outJsonData, generateMockData);
                // }
            }
            catch (e) {
        ...
        if (isInitial && !outData.getInitialDataSet) {
            outData = {};
            isInitial = false;
            outData.getInitialDataSet = createTenantDataLoader(mockDataRootFolder, entity, isDraft, dataAccess, [], // No base data fallback
            generateMockData);
        }
        if (isInitial) {
            ...
            if (forceNullableValuesToNull) {
                outData.__forceNullableValuesToNull = forceNullableValuesToNull;
            }
        }
        return outData;
    }
     ...
     * @param isDraft
     */
    constructor(rootFolder, entitySetDefinition, dataAccess, generateMockData, forceNullableValuesToNull, initializeMockData = true, isDraft = false) {
        ...
        if (initializeMockData) {
            this.readyPromise = MockDataEntitySet.read(rootFolder, entitySetDefinition.name, generateMockData, forceNullableValuesToNull, isDraft, dataAccess).then((mockData) => {
        ...
        if (!Object.prototype.hasOwnProperty.apply(this.contextBasedMockData, [contextId])) {
            if (this._rootMockDataFn) {
                ...
                if (typeof this._rootMockDataFn === 'function') {
                    const MockClass = this._rootMockDataFn;
                    ...
                    if (initialDataSetFn) {
                        (_a = mockDataFn.getInitialDataSet) !== null && _a !== void 0 ? _a : (mockDataFn.getInitialDataSet = initialDataSetFn); // It would exist in that case
                    }
                }
                else {
                    mockDataFn = { ...mockDataFn };
                }
                this.contextBasedMockData[contextId] = new functionBasedMockData_1.FunctionBasedMockData(mockDataFn, this.entityTypeDefinition, this, contextId);
            }
            else {
                this.contextBasedMockData[contextId] = new fileBasedMockData_1.FileBasedMockData(this._rootMockData, this.entityTypeDefinition, this, contextId);
            }
        }
        else {
    ...
    isDraft() {
        var _a, _b, _c, _d, _e;
        return !!((_c = (_b = (_a = this.entitySetDefinition) === null || _a === void 0 ? void 0 : _a.annotations.Common) === null || _b === void 0 ? void 0 : _b.DraftRoot) !== null && _c !== void 0 ? _c : (_e = (_d = this.entitySetDefinition) === null || _d === void 0 ? void 0 : _d.annotations.Common) === null || _e === void 0 ? void 0 : _e.DraftNode);
    }
    ...
    createTransformation(identifier, isProperty = false) {
        var _a, _b;
        if (typeof identifier === 'string') {
            const property = this.getProperty(identifier);
            if (property) {
                let type = property.type;
                if (type === 'Edm.String' && ((_b = (_a = property.annotations.Common) === null || _a === void 0 ? void 0 : _a.IsDigitSequence) === null || _b === void 0 ? void 0 : _b.valueOf()) === true) {
                    type = 'Edm.Int64';
                }
            ...
            const methodArgTransformed = identifier.methodArgs.map((methodArg, idx) => this.createTransformation(methodArg, identifier.propertyPaths && identifier.propertyPaths[idx] !== methodArg));
    ...
    checkLambdaExpression(expression, identifierTransformation, mockData, tenantId, odataRequest) {
        ...
        if ((_a = expression.expression) === null || _a === void 0 ? void 0 : _a.expressions) {
            expression.expression.expressions.forEach((entry) => {
                ...
                if (typeof entry.identifier === 'string') {
                    ...
                    entry.identifier.propertyPaths = entry.identifier.methodArgs.map((methodArg) => methodArg.replace(expression.key, replaceValue));
                }
            });
        }
        else {
    ...
    async performGET(keyValues, asArray, tenantId, odataRequest, dontClone = false) {
    ...
    async performPATCH(keyValues, patchData, tenantId, odataRequest, _updateParent = false) {
        ...
        for (const property of this.entityTypeDefinition.entityProperties) {
    ...
    async getEntityInterface(entitySet, serviceNameOrAlias, tenantId) {
