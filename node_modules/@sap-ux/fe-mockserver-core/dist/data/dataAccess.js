 ...
 */
class DataAccess {
    constructor(service, metadata, fileLoader, logger, serviceRegistry) {
        var _a;
        ...
        this.log = (_a = this.logger) !== null && _a !== void 0 ? _a : (0, logger_1.getLogger)('server:ux-fe-mockserver', this.debug);
    ...
    logRequest(message, odataRequest) {
    ...
    logResponse(message, odataRequest) {
        ...
        if (rootEntitySet) {
            ...
            if (actionName && actionName.length > 0) {
                ...
                if (actionDefinition) {
                    ...
                    const result = await mockEntitySet.executeAction(actionDefinition, actionData, odataRequest, odataRequest.queryPath[i - 1].keys);
                ...
                if (collecactionDefinition) {
                    ...
                    this.logRequest(`Action data: ${JSON.stringify(bodyActionData, null, 2)}`, odataRequest);
                    const result = await (await this.getMockEntitySet(entitySetName)).executeAction(collecactionDefinition, actionData, odataRequest, odataRequest.queryPath[i - 1].keys);
                    this.logResponse(`Response Data: ${JSON.stringify(result, null, 2)}`, odataRequest);
                    return result;
                }
                ...
                if ((functionDefinition === null || functionDefinition === void 0 ? void 0 : functionDefinition._type) === 'Function' || (functionDefinition === null || functionDefinition === void 0 ? void 0 : functionDefinition._type) === 'Action') {
                    ...
                    this.logRequest(`Action data: ${JSON.stringify(bodyActionData, null, 2)}`, odataRequest);
                    const result = await (await this.getMockEntitySet(entitySetName)).executeAction(functionDefinition, actionData, odataRequest, odataRequest.queryPath[i - 1].keys);
                    this.logResponse(`Response Data: ${JSON.stringify(result, null, 2)}`, odataRequest);
                    return result;
                }
            }
            ...
            if (actionDefinition) {
                if (actionDefinition.sourceType !== '') {
                        ...
                        if (!this.isV4()) {
                            ...
                            if (Array.isArray(outData)) {
                                outData = outData.map((element) => {
                                    return enrichElement(returnEntitySet !== null && returnEntitySet !== void 0 ? returnEntitySet : targetEntitySet, element);
                                });
                            }
                            else if (outData != null) {
                                outData = enrichElement(returnEntitySet !== null && returnEntitySet !== void 0 ? returnEntitySet : targetEntitySet, outData);
                            }
                        }
                    ...
                    this.logRequest(`Running action: ` + actionDefinition.fullyQualifiedName, odataRequest);
                    this.logRequest(`Action data: ${JSON.stringify(bodyActionData, null, 2)}`, odataRequest);
                    const result = await ((_b = (await mockEntityContainer_1.MockEntityContainer.read(this.mockDataRootFolder, odataRequest.tenantId, this.fileLoader, this))) === null || _b === void 0 ? void 0 : _b.executeAction(actionDefinition, Object.assign({}, actionData), odataRequest.queryPath[0].keys || {}, odataRequest));
                    this.logResponse(`Response Data: ${JSON.stringify(result, null, 2)}`, odataRequest);
                    return result;
                }
            }
                ...
                const result = (_e = (_d = (_c = (await mockEntityContainer_1.MockEntityContainer.read(this.mockDataRootFolder, odataRequest.tenantId, this.fileLoader, this))) === null || _c === void 0 ? void 0 : _c.handleRequest) === null || _d === void 0 ? void 0 : _d.call(_c, odataRequest)) !== null && _e !== void 0 ? _e : null;
                return result;
            }
        }
        return null;
     ...
     */
    applySelect(data, selectDefinition, expandDefinitions, entityType, odataRequest) {
    ...
    async applyExpandOptions(data, _entityType, navigationProperty, targetEntitySet, expandDefinition, odataRequest) {
        ...
        // $select
        this.applySelect(data[navigationProperty.name], expandDefinition.properties, expandDefinition.expand, navigationProperty.targetType, odataRequest);
    }
    /**
     ...
     */
    async getCrossServiceEntityInterface(serviceNameOrAlias, entityName, tenantId = 'tenant-default') {
        ...
        if (!otherService) {
            throw new Error(`Service '${serviceNameOrAlias}' not found in registry. Available services: ${this.serviceRegistry.getServicesWithAliases()}`);
        }
     ...
     */
    async getData(odataRequest, dontClone = false, ignoreRequestOperation = false) {
        var _a, _b, _c, _d, _e;
        ...
        let data = await odataRequest.queryPath.reduce(async (inData, queryPathPart, index) => {
                ...
                if (navPropDetail) {
                    if (navPropDetail.name === 'SiblingEntity' && currentEntityType) {
                        ...
                        if (asArray) {
                            ...
                            this.logRequest(`Lookup on navigation property: ${navPropDetail.name} with keys ${JSON.stringify(currentKeys)}`, odataRequest);
                        }
                        ...
                        if (navPropDetail.referentialConstraint.length == 0 &&
                            ...
                            if (this.metadata.getVersion() === '2.0') {
                                ...
                                currentEntitySet = currentEntitySet === null || currentEntitySet === void 0 ? void 0 : currentEntitySet.navigationPropertyBinding[navPropDetail.name];
                            }
                            else {
            ...
            return (await this.getMockEntitySet(currentEntitySet === null || currentEntitySet === void 0 ? void 0 : currentEntitySet.name, this.generateMockData, undefined, targetContainedEntityType, targetContainedData)).performGET(currentKeys, asArray, odataRequest.tenantId, odataRequest, dontClone);
        }, Promise.resolve({}));
        ...
        if (data) {
            ...
            if (!dontClone) {
                ...
                this.applySelect(data, odataRequest.selectedProperties, odataRequest.expandProperties, currentEntityType, odataRequest);
            }
    ...
    async updateData(odataRequest, patchData) {
        ...
        this.logRequest(`PATCH details - Keys: ${JSON.stringify(odataRequest.queryPath[0].keys)}, Tenant: ${odataRequest.tenantId}, Data: ${JSON.stringify(patchData, null, 2)}`, odataRequest);
        const resultData = await mockEntitySet.performPATCH(odataRequest.queryPath[0].keys, patchData, odataRequest.tenantId, odataRequest, true);
    ...
    async createData(odataRequest, postData) {
        ...
        if (odataRequest.queryPath.length > 1 && parentEntitySet) {
            ...
            if (navPropDetail) {
                ...
                const data = await (await this.getMockEntitySet(parentEntitySet.name)).performGET(odataRequest.queryPath[odataRequest.queryPath.length - 2].keys, false, odataRequest.tenantId, odataRequest, true);
                const providedKeys = {};
            ...
            postData = await mockEntitySet.performPOST(currentKeys, postData, odataRequest.tenantId, odataRequest, true);
    ...
    addV2Metadata(entitySet, currentKeys, postData, dataType) {
        var _a;
        const propertyKeys = entitySet.entityType.keys;
        const keyStr = this.getV2KeyString(currentKeys, propertyKeys);
        let urlPath = (_a = this.service.urlPath) !== null && _a !== void 0 ? _a : '';
        if (!urlPath.endsWith('/')) {
            urlPath += '/';
        }
    ...
    _applyOrderBy(data, orderByDefinition, entitySetDefinition) {
        data.sort(function (firstElement, secondElement) {
            ...
            orderByDefinition.forEach((orderByDef) => {
                ...
                const propertyDef = entitySetDefinition === null || entitySetDefinition === void 0 ? void 0 : entitySetDefinition.getProperty(orderByDef.name);
                let firstElementData = (0, common_1.getData)(firstElement, orderByDef.name);
                let secondElementData = (0, common_1.getData)(secondElement, orderByDef.name);
                if (propertyDef) {
                    if (propertyDef.type === 'Edm.String' && ((_a = propertyDef.annotations.Common) === null || _a === void 0 ? void 0 : _a.IsDigitSequence)) {
    ...
    async _applyGroupBy(data, applyDefinition, odataRequest, mockData, currentEntityType) {
        const dataByGroup = {};
                ...
                if (applyDefinition.subTransformations.length > 0) {
                    ...
                    aggregateDefinition.aggregateDef.forEach((subAggregateDefinition) => {
                        ...
                        if (subAggregateDefinition.operator === undefined &&
                            ...
                            const aggregatedProperty = subAggregateDefinition.sourceProperty === 'count'
                                ? undefined
                                : currentEntityType.entityProperties.find((prop) => prop.name === subAggregateDefinition.sourceProperty);
                            const isCurrency = ((_c = (_b = (_a = aggregatedProperty === null || aggregatedProperty === void 0 ? void 0 : aggregatedProperty.annotations) === null || _a === void 0 ? void 0 : _a.Common) === null || _b === void 0 ? void 0 : _b.IsCurrency) === null || _c === void 0 ? void 0 : _c.valueOf()) === true;
    ...
    async applyTransformation(transformationDef, data, odataRequest, mockEntitySet, mockData, currentEntityType) {
        switch (transformationDef.type) {
            ...
            case 'aggregates':
                ...
                }, odataRequest, mockData, currentEntityType);
                break;
            case 'ancestors':
                this.logRequest('Applying ancestors transformation ' +
                    JSON.stringify(transformationDef.parameters.inputSetTransformations[0]), odataRequest);
                const limitedHierarchyForAncestors = await this.applyTransformation(transformationDef.parameters.inputSetTransformations[0], await mockData.getAllEntries(odataRequest), odataRequest, mockEntitySet, mockData, currentEntityType);
                this.logRequest(`Limited hierarchy for ancestors ${(0, util_1.inspect)(limitedHierarchyForAncestors)}`, odataRequest);
                ...
                this.logRequest(`Ancestors data ${(0, util_1.inspect)(data)}`, odataRequest);
                break;
            case 'skip':
                data = [];
                break;
            ...
            case 'descendants':
                ...
                const limitedHierarchyData = await this.applyTransformation(transformationDef.parameters.inputSetTransformations[0], await mockData.getAllEntries(odataRequest), odataRequest, mockEntitySet, mockData, currentEntityType);
                this.logRequest(`Limited hierarchy data ${(0, util_1.inspect)(limitedHierarchyData)}`, odataRequest);
                ...
                this.logRequest(`Descendants data ${(0, util_1.inspect)(data)}`, odataRequest);
                break;
            case 'groupBy':
            ...
            case 'customFunction':
                if (transformationDef.name === 'com.sap.vocabularies.Hierarchy.v1.TopLevels') {
                    this.logRequest(`Applying custom function 'com.sap.vocabularies.Hierarchy.v1.TopLevels'`, odataRequest);
                    this.logRequest(`Parameters: ${(0, util_1.inspect)(transformationDef.parameters)}`, odataRequest);
                    data = await mockData.getTopLevels(data, transformationDef.parameters, odataRequest);
                    this.logRequest(`Top Level response: ${(0, util_1.inspect)(data)}`, odataRequest);
                }
                break;
        }
        return data;
