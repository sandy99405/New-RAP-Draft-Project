...
const sessionContextMiddleware = require('./middleware/session-context-middleware');
...
const partitioned = require('./utils/partitioned');
const { STATE_PARAMETER_PATH } = require('./passport/utils');

const UGW_SESSION_CONTEXT_PATH = '/ugw-session-context';
...
module.exports = function bootstrap(options) {
  ...
  if (process.env.UGW_GET_SESSION_ATTRIBUTES === 'true') {
    app.use(UGW_SESSION_CONTEXT_PATH, bodyParser.json());
    app.use(UGW_SESSION_CONTEXT_PATH, sessionContextMiddleware);
  }
  ...
  app.use(subscriptionUtils.getSMSCallbackPath().onSubscriptionPrefixZoneId, bodyParser.json());
  ...
  if (routerConfig.cookies){
    if (routerConfig.cookies['SameSite']) {
