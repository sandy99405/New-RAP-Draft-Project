...
function getVcapServicesData(req, appKey, cb) {
  ...
  getHTML5RepoToken(req, html5RepoCredentials, function (err, html5RepoToken) {
    ...
    if (appKey && appKey.appPrefix && !isOwnSapCloudService(appKey.appPrefix, req)) {
...
function getApplicationKey(req,segmentIndex) {
  ...
  } else if (parts.length === 3 && isOwnSapCloudService(parts[1], req)) {
    appKey.appPrefix =  parts[1];
    appKey.appNameWithoutPrefix = parts[2];
  } else {
...
function isOwnSapCloudService(appPrefix, req) {
  const routerConfig = req && req.app && req.app.get && req.app.get('mainRouterConfig');
  const ownSapCloudService = routerConfig && routerConfig.ownSapCloudService;
  return ownSapCloudService && ownSapCloudService.includes(appPrefix);
}
