...
const applicationUrlParam                        = 'application.url';
...
function getSMSApplicationUrl(req,smsCredentials, cb){
  ...
  iasUtils.getIasConfig(req, function (err, iasOptions) {
    ...
    const appUrl = getApplicationUrlFromParam(req, iasOptions.tenant, req.body.subscriber.subaccountSubdomain);
    if (appUrl) {
      return cb(null, {applicationURL: appUrl});
    }

    let approuterHost = urlUtils.getRedirectHost(req);
    const certSegment = ['.cert.' , '.internal.'].find(segment => approuterHost.includes(segment));
    approuterHost = approuterHost.replace(certSegment,'.');
...
function getSaaSRegistryApplicationUrl(req,cb){
  ...
  uaaUtils.getUaaConfig(req, function (err, uaaOptions) {
    ...
    const appUrl = getApplicationUrlFromParam(req, uaaOptions.tenant, subdomain);
    if (appUrl) {
      return cb(null, appUrl);
    }
...
function getSMSCallbackPath(credentials){
  const callBackPath = {
    getDependenciesPrefix: '/v1.0/callback/tenants',
    onSubscriptionPrefix: '/v1.0/callback/tenants',
    getDependenciesPrefixZoneId: '/v1.0/callback/zones',
    onSubscriptionPrefixZoneId: '/v1.0/callback/zones'
  };
  if (!credentials){
  ...
  try {
    ...
    const parsedDependencyCallbackUrl = urijs.parse(appUrlsJson.dependenciesCallbacks.url);
    callBackPath.getDependenciesPrefix = getSMSCallbackPrefix(parsedDependencyCallbackUrl);

    const parsedSubscriptionCallbacksUrl = urijs.parse(appUrlsJson.subscriptionCallbacks.url);
    callBackPath.onSubscriptionPrefix = getSMSCallbackPrefix(parsedSubscriptionCallbacksUrl);
  } catch (e){
...
function getSMSCallbackPrefix(parsedUrl) {
  let index = parsedUrl.path.indexOf('/{zoneId}');
  if (index === -1) {
    index = parsedUrl.path.indexOf('/{app_tid}');
  }
  if (index > 0){
    return parsedUrl.path.substring(0,index);
  }
...
function getApplicationUrlFromParam(req, providerSubdomain, subscriberSubdomain) {
  logger.info(`Fetching application url parameter for reqest url ${req.url} and provider subdomain ${providerSubdomain} and subscriber subdomain ${subscriberSubdomain}`);
  const urlParam = req && providerSubdomain && subscriberSubdomain && urlUtils.getQueryParam(req,applicationUrlParam);
  if (!urlParam) {
    logger.info(`No application url parameter found for reqest url ${req.url}`);
  } else {
    logger.info(`Application url parameter found ${urlParam}`);
    const subscriberUrl =  urlParam.replace(providerSubdomain, subscriberSubdomain);
    logger.info(`Subscriber url ${subscriberUrl}`);
